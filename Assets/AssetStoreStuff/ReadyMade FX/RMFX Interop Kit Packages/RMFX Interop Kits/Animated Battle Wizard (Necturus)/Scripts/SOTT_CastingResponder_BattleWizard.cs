...
public class SOTT_CastingResponder_BattleWizard : BattleWizardResponderBase
{
	...
	{
		if (elapsedTime > warmupDuration+1.5f)
		{
	...
	void CueAnimation_CALLBACK()
	{
		...
		if (anim != null)
		{
			for (int i = 0; i < animationClipSeries.Length; i++)
			{
				AnimationState animState = anim.CrossFadeQueued(animationClipSeries[i].name, animationOverlap, (i == 0) ? QueueMode.PlayNow : QueueMode.CompleteOthers);
				animState.speed = animationSpeed;
				animState.wrapMode = WrapMode.Once;

				if (i == 0)
				{
					hold_animState = animState;
				}
			}
		}
	}
